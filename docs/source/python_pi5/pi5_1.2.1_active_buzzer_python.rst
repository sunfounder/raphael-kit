...
In this experiment, an active buzzer, an NPN transistor, and a 1kΩ resistor are used. The resistor is placed between the GPIO pin and the transistor’s base to protect the transistor. When GPIO17 on the Raspberry Pi is set to a low level (0V), the transistor saturates and conducts, allowing the buzzer to sound. When GPIO17 is set to a high level, the transistor is turned off, and the buzzer remains silent.
