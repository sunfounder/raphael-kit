...
このプロジェクトでは、NPNトランジスタを使用してアクティブブザーを鳴らす方法を学びます。
...
この実験では、アクティブブザー、NPNトランジスタ、および1kΩの抵抗を使用します。抵抗はGPIOピンとトランジスタのベースの間に接続されており、ベース電流を制限してトランジスタを保護します。Raspberry PiのGPIO17が高レベル（3.3V）を出力すると、トランジスタが飽和領域に入り、電流がブザーを流れて音が鳴ります。GPIO17が低レベル（0V）を出力すると、トランジスタは遮断され、ブザーは鳴りません。
