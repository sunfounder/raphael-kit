...
#. In the main loop, it checks the state of the micro switch. If pressed, LED2 turns on and LED1 off. If not pressed, LED2 turns off and LED1 on. The loop repeats every 0.5 seconds. Catches a KeyboardInterrupt (like Ctrl+C) to allow for graceful script termination.
