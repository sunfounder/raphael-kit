...
#. Crea una instancia de ``Picamera2`` y recupera la configuración predeterminada de vista previa.
...
#. Establece el tamaño y el formato de la vista previa de la cámara.  
   El tamaño se establece en 800x600 píxeles y el formato en ``XRGB8888``.
...
#. Captura una imagen desde la cámara y la guarda en ``~/``.
   ...
   .. note::
       Puedes usar la función ``camera.capture()`` junto con un bucle ``for`` para lograr disparos continuos.  
       Usa la función ``delay`` para ajustar el intervalo de tiempo entre fotos.
...
#. Este bloque de código maneja una interrupción de teclado (como Ctrl+C) deteniendo la vista previa de la cámara.  
   La instrucción ``pass`` se utiliza para manejar la excepción sin realizar ninguna acción adicional.
